...
// ============== BRIDGE ADDRESSES ==============
// STARKNET TO BITCOIN: StarkNet tokens move to this address when bridging to Bitcoin
export const STARKNET_TO_BTC_ADDRESS = '0x01ebd9b12b7477bd358ab10fd9ed0f00b1f2cf4d6dfaeb8881567906af8c16d9';
export const STARKNET_LIQUIDITY_POOL_ADDRESS = STARKNET_TO_BTC_ADDRESS;
...
// Legacy support - BTC_TOKEN_ADDRESS now references the correct direction
export const BTC_TOKEN_ADDRESS = BTC_TO_STARKNET_ADDRESS;
...
// Global network mode
let currentNetworkMode: 'mainnet' | 'testnet' = 'testnet';
...
/**
 * BTC → Starknet bridge — Step 1 of 2 (frontend responsibility).
 *
 * Sends BTC from the user's Bitcoin wallet to the bridge pool address and
 * returns the Bitcoin txHash.  The backend relayer watches for the on-chain
 * confirmation and then calls mint_rawbtc on the Starknet contract.
 * The frontend must register the bridge with the backend (POST /bridges)
 * and poll GET /bridges/:id until status === "completed".
 */
export async function bridgeBtcToToken(
  amount: string,
  btcAddress: string,
  _tokenOut: string,        // kept for API compatibility, relayer decides token
  _minAmountOut: string,    // kept for API compatibility
  to: string,               // Starknet recipient address
  ...
  console.log('🔄 BTC → Starknet: sending BTC via wallet, relayer will mint STRK after confirmation');
  ...
  console.log('✅ BTC sent — txHash:', btcApproval.tx_hash);
  console.log('⏳ Relayer will mint rawBTC on Starknet once the transaction confirms on-chain');
  ...
  return {
    ...
    // transaction_hash is unknown until the relayer mints — will be returned via backend polling
    transaction_hash: null as null,
  };
}
      ...
      case 'unisat': {
        if (typeof window !== 'undefined' && (window as any).unisat) {
          ...
          try {
            ...
            // Use the wallet's native address as source and bridge address as destination
            const sourceAddress = walletAccount;
            const destinationAddress = BTC_TO_STARKNET_ADDRESS; // Bitcoin to Starknet bridge address
            
            // Memo indicating bridge operation
            const memo = `Bridge to Starknet`;
            ...
            // Execute transaction to bridge address
      ...
      default: {
        ...
        if (typeof window !== 'undefined' && (window as any).bitcoin) {
          ...
          try {
            ...
            const amountSatoshis = Math.floor(parseFloat(amount) * 100000000);
            const bridgeAddress = BTC_TO_STARKNET_ADDRESS;
