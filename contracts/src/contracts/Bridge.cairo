...
pub mod Bridge {
    ...
    /// Release STRK from the bridge pool to a user (called by admin after BTC deposit confirmed).
    /// STRK flows: bridge_contract_pool → user_starknet_address.
    #[external(v0)]
    fn release_strk(
        ref self: ContractState,
        to: ContractAddress,
        ...
        // Call STRK ERC20 transfer — moves STRK from this contract's own balance to `to`
        let strk_token: ContractAddress = STRK_TOKEN.try_into().unwrap();
        let mut call_data = ArrayTrait::new();
        call_data.append(to.into());
        call_data.append(amount.low.into());
        call_data.append(amount.high.into());
        call_contract_syscall(strk_token, selector!("transfer"), call_data.span()).unwrap_syscall();

        self.emit(Event::RawBTCMinted(RawBTCMinted { to, amount, btc_tx_hash }));
    }
