...
export default function SwapPage() {
  ...
  const computeRate = (from: string, to: string, amount: number): number => {
    if (amount === 0) return 0;
    let baseRate = 0.0542;
    if (from === 'BTC' && to === 'ETH') baseRate = 18.45;
    else if (from === 'ETH' && to === 'USDC') baseRate = 3425;
    else if (from === 'USDC' && to === 'ETH') baseRate = 1 / 3425;
    else if (from === 'BTC' && to === 'USDC') baseRate = 0.0542 * 3425;
    else if (from === 'USDC' && to === 'BTC') baseRate = 1 / (0.0542 * 3425);
    if (amount > 10) baseRate *= 0.998;
    else if (amount > 1) baseRate *= 0.999;
    return baseRate;
  };
  ...
  const handleAmountChange = (value: string, type: 'from' | 'to') => {
    ...
    if (type === 'from') {
      ...
      const rate = computeRate(fromToken, toToken, numValue);
      ...
      setToAmount(value);
      const rate = computeRate(fromToken, toToken, numValue);
      updateSwapMetrics(rate > 0 ? numValue / rate : 0, fromToken, toToken);
      if (numValue > 0 && rate > 0) {
