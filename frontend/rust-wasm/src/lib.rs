...
pub mod constants {
    ...
    pub const STARKNET_TO_BTC_BRIDGE_ADDRESS: &str =
        "0x033d11992f96518e13dc34d6b8ec10c2c5c9c1d220a39a274b46a58ee4fab4de";
    pub const BTC_TO_STARKNET_BRIDGE_ADDRESS: &str = "tb1q5x90tfa7acp3rtn826q2ndmqexudpgcaygga6j";
}
...
impl Bridge {
    ...
    fn starknet_to_btc_bridge_address(&self) -> ContractAddress {
        constants::STARKNET_TO_BTC_BRIDGE_ADDRESS.to_string()
    }

    fn btc_to_starknet_bridge_address(&self) -> String {
        constants::BTC_TO_STARKNET_BRIDGE_ADDRESS.to_string()
    }
    ...
    fn verify_bitcoin_transaction(&self, tx_hex: &str, block_hash: &str, merkle_proof: &str) -> Result<bool, String> {
        ...
        let bridge_address = constants::BTC_TO_STARKNET_BRIDGE_ADDRESS;
    ...
    pub fn bridge_btc_to_token(
        ...
        let bridge_address = self.btc_to_starknet_bridge_address();
        ...
        self.record_transaction(&caller, TransactionType::BridgeBTCToToken, token_out.clone(), amount, 0, bridge_address.clone(), btc_address.clone(), swap_id);
        self.emit(SimpleEvent { event_type: "Swapped".to_string(), data: format!("router:,token_in:,token_out:{},amount_in:{},amount_out:0,to:{}", token_out, amount, bridge_address) });
        Ok(swap_id)
    }
    ...
    pub fn bridge_token_to_btc(
        ...
        let bridge_address = self.starknet_to_btc_bridge_address();
        self.assert_not_paused().map_err(JsValue::from)?;
        self.ensure(!self.storage.emergency_paused, "EMERGENCY_PAUSED").map_err(JsValue::from)?;
        ...
        self.record_transaction(&caller, TransactionType::BridgeTokenToBTC, token_in.clone(), amount_in, 0, bridge_address.clone(), btc_address_str.clone(), swap_id);
        self.emit(SimpleEvent { event_type: "Swapped".to_string(), data: format!("router:,token_in:{},token_out:,amount_in:{},amount_out:{},to:{}", token_in, amount_in, min_btc_out, bridge_address) });
        Ok(swap_id)
    }
    ...
    pub fn initiate_bitcoin_deposit(
        ...
        self.record_transaction(&caller, TransactionType::Deposit, String::new(), amount, 0, bridge_address, btc_address, deposit_id);
        Ok(deposit_id)
    }
